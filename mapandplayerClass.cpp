...
int main()
{
    ...
    while (!GetAsyncKeyState(VK_ESCAPE) && exit==0)
    {
        ...
        if (pl.score<10 && pl.score>=0) // собираем кристаллы
        {
            txTextOut(600, 40,"Cristalls: " );
            txTextOut(750, 40,scr );
        }
		else if (pl.score==-1) // если очки -1 то игра проиграна
		{
			txTextOut(600, 40,"You Loose!!!" );
            Sleep(1000);
            GameOver();
            break;
		}
        else // переключение на следующий уровень
        {
...
void Map::draw()
{
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<m; j++)
        {
            ...
            {
                txTransparentBlt(txDC(), j*50, i*50, 50, 50, sprite, 100, 50, TX_WHITE);
            }
...
void player::Right()
{
    ...
    if (mp->get_cell(x+38, y+64)!=-1 && mp->get_cell(x+38, y+64)!=0)
    {
		...
		if (mp->get_cell(x+38, y+64)==5) // проверяем если ловушка то заменяем на пустую и  у
        {
            mp->set_cell(x+38, y+64,1);
            score=-1; // убираем очки чтобы понять что игра проиграна
        }
...
void player::Left()
{
    ...
    if (mp->get_cell(x+26, y+64)!=-1 && mp->get_cell(x+26, y+64)!=0)
    {
        ...
        {
            mp->set_cell(x+26, y+64,1);  // устанавливаем ячейку без кристала
            GetCristall();
            score++;  // увеличиваем очки
        }

		if (mp->get_cell(x+26, y+64)==5) // проверяем если ловушка то заменяем на пустую и  у
        {
...
void player::Up()
{
    ...
    if (mp->get_cell(x+32, y+55)!=-1 && mp->get_cell(x+32, y+55)!=0 )
    {
		...
		if (mp->get_cell(x+32, y+55)==5) // проверяем если ловушка то заменяем на пустую и  у
        {
            mp->set_cell(x+32, y+55,1);
            score=-1; // убираем очки чтобы понять что игра проиграна
        }
...
void player::Down()
{
    ...
    if (mp->get_cell(x+32, y+67)!=-1 && mp->get_cell(x+32, y+67)!=0)
    {
		...
		if (mp->get_cell(x+32, y+67)==5) // проверяем если ловушка то заменяем на пустую и  у
        {
            mp->set_cell(x+32, y+67,1);
            score=-1; // убираем очки чтобы понять что игра проиграна
        }
